...
input_tiff = "Cat_1.tif"; target_floor_rel = 0.05; alpha = 0.5
c_list = [0.0, 300.0, 600.0, 1000.0, 1500.0, 2200.0, 3000.0, 5000.0]  # 0 == random baseline
...
for c in c_list:
    runs[c] = run(c)
    r = runs[c]
    tag = "random" if c == 0.0 else f"c={c:g}"
    print(f"  {tag:10s} RMSE={r['RMSE'][-1]:8.4f}  vort={r['NUM'][-1]:5d}", flush=True)
...
plt.xlabel("quadratic strength c (0 = random)"); plt.ylabel("final RMSE (SR)")
...
panels = [("target", F1), ("random", runs[0.0]["I_final"]), (f"c={best_c:g}", runs[best_c]["I_final"])]
