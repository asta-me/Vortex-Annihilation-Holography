#%%
"""
TEMP: iterated projection (multi-pass Poisson per GS iteration).

One Poisson solve removes the least-squares (first-order) vorticity, but a few residual vortices
remain. Applying the projector K times per GS iteration (re-detect wrapped gradient of the blended
phase, re-solve) should drive the residual vortex count to ~0 more cleanly, at O(K*N log N) cost.
We sweep K. Projector alpha fixed. RMSE + vortices reported.
...
output_dir = os.path.join(script_dir, "output_temp_iterated"); os.makedirs(output_dir, exist_ok=True)
...
def project_crop(pc, K):
    for _ in range(K):
        psi = irrotational_phase(pc); psi = psi + cp.angle(cp.sum(cp.exp(1j*(pc-psi))))
        pc = cp.angle((1-alpha)*cp.exp(1j*pc) + alpha*cp.exp(1j*psi))
    return pc
...
def run_K(K):
    ...
    for i in range(1, loop):
        ...
        pn = pha.copy(); pn[sr_r0:sr_r1, sr_c0:sr_c1] = project_crop(pc, K); phi = cp.exp(1j*pn)
    return dict(RMSE=RMSE, NUM=NUM, I_final=final_reconstruction(E2_k))

#%% Run
print(f"Iterated projection on {input_tiff} (alpha={alpha}, loop={loop})", flush=True)
runs = {}
for K in K_list:
    runs[f"K{K}"] = run_K(K)
    r = runs[f"K{K}"]
    print(f"  K={K}  RMSE={r['RMSE'][-1]:8.4f}  vort={r['NUM'][-1]:5d}", flush=True)

fig = plt.figure()
for name, r in runs.items():
    plt.plot(r["NUM"][1:], label=name)
plt.xlabel("Iteration"); plt.ylabel("Vortex count (SR)"); plt.title(f"Iterated projection — vortices ({os.path.splitext(input_tiff)[0]})")
