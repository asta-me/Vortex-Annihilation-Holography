#%%
"""
TEMP: keep the blended-field AMPLITUDE (soft annihilation) vs phase-only blend.

The convex blend field = (1-a)e^{i*pha} + a*e^{i*psi} has |field|<1 at vortex cores -- physically a
real field DOES dip to zero at a vortex. We normally discard that amplitude (phi = e^{i*angle}).
Here we instead carry the complex blended field (amplitude dip kept) into the SR, a "soft"
annihilation, and compare with the standard phase-only blend. Projector alpha fixed.
...
output_dir = os.path.join(script_dir, "output_temp_blend"); os.makedirs(output_dir, exist_ok=True)
...
def run(keep_amp):
    ...
    for i in range(1, loop):
        ...
        field = (1-alpha)*cp.exp(1j*pc) + alpha*cp.exp(1j*psi)      # complex blend, |field|<=1
        phi = cp.exp(1j*pha)
        if keep_amp:
            phi[sr_r0:sr_r1, sr_c0:sr_c1] = field                  # keep amplitude dip (soft)
        else:
            phi[sr_r0:sr_r1, sr_c0:sr_c1] = cp.exp(1j*cp.angle(field))  # phase-only (standard)
    return dict(RMSE=RMSE, NUM=NUM, I_final=final_reconstruction(E2_k))

#%% Run
print(f"Blend amplitude study on {input_tiff} (alpha={alpha}, loop={loop})", flush=True)
runs = {"phase_only": run(False), "keep_amplitude": run(True)}
for name, r in runs.items():
    print(f"  {name:14s} RMSE={r['RMSE'][-1]:8.4f}  vort={r['NUM'][-1]:5d}", flush=True)

fig = plt.figure()
for name, r in runs.items():
    plt.plot(r["RMSE"][1:], label=name)
plt.xlabel("Iteration"); plt.ylabel("RMSE (SR)"); plt.title(f"Blend: phase-only vs keep-amplitude — {os.path.splitext(input_tiff)[0]}")
