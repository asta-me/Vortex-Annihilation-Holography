#%%
"""
TEMP: MRAF (Mixed-Region Amplitude Freedom) + projector, vs the standard hard-constraint GS.

Diagnosis (temp_residual_diagnosis.py) showed the residual error at convergence is NOT vortices
but a broadly-distributed error in the bright signal region -> a phase-retrieval limitation. MRAF
attacks exactly this: instead of hard-constraining the amplitude everywhere, it
    - softens the SIGNAL region:  A_SR <- m*target + (1-m)*|propagated|   (m in (0,1]),
    - leaves the NOISE region FREE (no energy renormalization),
so the free noise region can absorb the error and the signal region converges much better.

We keep the irrotational projector (alpha) ON in all runs (we still want vortex-free phase), and
sweep the mixing parameter m. Reference: the standard pipeline (hard SR + El noise energy).
...
m_list = [1.0, 0.7, 0.5, 0.4, 0.3]
n = m = 512; nn = n + 2*(n//4); mm = m + 2*(m//4)
script_dir = os.path.dirname(os.path.abspath(__file__))
output_dir = os.path.join(script_dir, "output_temp_mraf"); os.makedirs(output_dir, exist_ok=True)
...
def rmse_of(ap):
    I = ap[sr_r0:sr_r1, sr_c0:sr_c1]**2; I = E_gpu*I/(cp.sum(I)+1e-12)
    return float(cp.sqrt(cp.mean((I-F1_gpu)**2)).get())


def run_standard():
    """Original pipeline: hard SR amplitude + noise-region energy El. Projector ON."""
    cp.random.seed(seed)
...
def run_mraf(mmix):
    """MRAF: SR softened by mixing m (SR = m*target + (1-m)*propagated, both energy-normalized),
    noise region kept at energy El (shape free). Projector ON."""
    cp.random.seed(seed)
    phi = cp.exp(1j*2*cp.pi*cp.random.rand(nn,mm)); amp = cp.random.rand(nn,mm)
    RMSE = np.zeros(loop); NUM = np.zeros(loop, dtype=int)
    for i in range(1, loop):
        ...
        po, ne = function_vortex_detection_accegpu(pha[sr_r0:sr_r1, sr_c0:sr_c1], dh, use_cupy=True); NUM[i]=po+ne
        phi = cp.exp(1j*project_phase(pha))
        # MRAF amplitude update: normalize each region to consistent energy, then MIX the SR.
        a_in = bandlim_in*ap; a_ou = bandlim_ou*ap
        a_in_n = cp.sqrt(E_gpu)*(a_in/(cp.sqrt(cp.sum(a_in**2))+1e-12))     # SR propagated, energy E
        a_ou_n = cp.sqrt(El_gpu)*(a_ou/(cp.sqrt(cp.sum(a_ou**2))+1e-12))    # NR, energy El (shape free)
        amp = bandlim_in*(mmix*F_gpu + (1.0-mmix)*a_in_n) + a_ou_n
    return dict(RMSE=RMSE, NUM=NUM, I_final=final_reconstruction(E2_k))


#%% Run
print(f"MRAF sweep on {input_tiff} (alpha={alpha}, loop={loop})", flush=True)
runs = {"standard": run_standard()}
for mm_ in m_list:
    runs[f"mraf_m{mm_}"] = run_mraf(mm_)
for name, r in runs.items():
    print(f"  {name:12s} RMSE={r['RMSE'][-1]:8.4f}  vort={r['NUM'][-1]:5d}", flush=True)

#%% Plots
fig = plt.figure()
for name, r in runs.items():
    plt.plot(r["RMSE"][1:], label=name)
plt.xlabel("Iteration"); plt.ylabel("RMSE (SR)"); plt.title(f"MRAF vs standard — {os.path.splitext(input_tiff)[0]}")
